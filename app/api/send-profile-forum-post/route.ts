...
function buildInterestLines(profile) {
  ...
  return profile.sections.map(section => {
    ...
    // Custom formatting for known types
...
export async function POST(request: NextRequest) {
  ...
  try {
    console.log('[ForumPost] Received:', { user, profile });
    ...
    const content = generateProfilePost(user, profile);
    const forumPost = await whopApi
      .withUser(process.env.WHOP_AGENT_USER_ID)
      .createForumPost({
        input: {
          forumExperienceId,
          title: `👋 Meet ${user.name} (@${user.username})`,
          content,
        },
      });
